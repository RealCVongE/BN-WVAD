import torch
import numpy as np
from dataset_loader_my import myVideo
...
def get_predict(test_loader, net):
    ...
    # for i in range(len(test_loader.dataset)//5):  # test 갯수가 5개 미만이면 0 추출이 없음
...
def get_sub_metrics(frame_predict, frame_gt):
    anomaly_mask = np.load('/home/bigdeal/mnt2/BN-WVAD/frame_label/my_anomaly_mask_1batch_50_ab.npy')
...
def test(net, test_loader, test_info, step, model_file = None):
    with torch.no_grad():
        ...
        frame_gt = np.load("/home/bigdeal/mnt2/BN-WVAD/frame_label/my_gt_data_1batch_50_ab.npy")
