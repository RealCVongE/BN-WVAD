...
if __name__ == "__main__":
    ...
    args.log_path = os.path.join(args.log_path, time_ymd, 'my', args.version)
    args.model_path = os.path.join(args.model_path, time_ymd, 'my', args.version)
    ...
    normal_train_loader = data.DataLoader(
        myVideo(root_dir = args.root_dir, mode = 'Train', num_segments = args.num_segments, len_feature = args.len_feature, is_normal = True),
            batch_size = args.batch_size,
            shuffle = True, num_workers = args.num_workers,
            worker_init_fn = worker_init_fn, drop_last = True)
    abnormal_train_loader = data.DataLoader(
        myVideo(root_dir = args.root_dir, mode='Train', num_segments = args.num_segments, len_feature = args.len_feature, is_normal = False),
            batch_size = args.batch_size,
            shuffle = True, num_workers = args.num_workers,
            worker_init_fn = worker_init_fn, drop_last = True)
    ...
    for step in tqdm(
        ...
        if step % args.plot_freq == 0 and step > 0:
            ...
            if test_info["AP"][-1] > best_scores['best_AP']:
                utils.save_best_record(test_info, os.path.join(args.log_path, "my_best_record_{}.txt".format(args.seed)))

                torch.save(net.state_dict(), os.path.join(args.model_path, "my_best_{}.pkl".format(args.seed)))
