...
class UCFVideo(data.DataLoader):
    def __init__(self, root_dir, mode, num_segments, len_feature, seed=-1, is_normal=None):
        ...
        split_path = os.path.join("list",'ucf-i3d_{}.list'.format(self.mode))
        split_file = open(split_path, 'r',encoding="utf-8")
        self.vid_list = []
        for line in split_file:
            if os.path.exists(os.path.join(self.feature_path, line.split()[0])):
        ...
        if self.mode == "Train":
            if is_normal is True:
                # 후반부 데이터 (정상 데이터) 사용
                self.vid_list = self.vid_list[1400:]
